...
# For webcam input:
cap = cv2.VideoCapture(r'F:\work\M.6\0.ME\ysc\code\phase 2_3D MediaPipe\Pesa\Video\2_pos\S.mp4')
...
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    try:
        ...
        LPos.setCord(int(LeftHeelVal.x*width), int(LeftHeelVal.y*height))
        
        #print('position:', LPos['x'])
        ...
        RPos.setCord(int(RightHeelVal.x*width), int(RightHeelVal.y*height))
        ...
        #     if RPos[1]<lock_foot:
        #         stage_r = 'up'
        #     if RPos[1]>lock_foot and stage_r =='up':
        #         stage_r= 'down'
        #         show_r = "rx: "+str(RPos[0])
        #         result_distance_r.append(abs(RPos[0]-distance_r))
        #         distance_r = RPos[0]
        #         counter_r+=1
        #         print("R: ",counter_r)
        #         print("Rx: ",RPos[0])
