...
def handcommand():
        print("hand!!")
        from handTracker import Hz
        if Hz is not None:
            Hz_model[0] = Hz

def shouldercommand():
        print("shoulder!!")
        # runpy.run_module('shoulderTracker')
        from shoulderTracker import avg_value
...
def gaitcommand():
        print("gait!!")
        from footTracker import status
        if status == "Normal":
...
def Diagnosecommand():
    ...
    if T_name + U_name + S_name >= 2:
        messagebox.showinfo(title="Diagnose", message= str(T) + str(U)+ str(S) + '\nParkinson')
