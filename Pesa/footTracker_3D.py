...
# For webcam input:
cap = cv2.VideoCapture(r'F:\work\M.6\0.ME\ysc\code\phase 2_3D MediaPipe\Pesa\Video\pos_1\U.mp4')
...
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    try:
        ...
        midEye = ((LEye[0] + REye[0])/2, (LEye[1] + REye[1])/2)
        midFoot = ((LPos.x + RPos.x)/2, (LPos.y + RPos.y)/2)
        
        localHeight = abs(midEye[1] - midFoot[1]) # int(math.dist(midEye ,midFoot))
...
# avr = sum(distance) / len(distance)
# for i in distance:
#     if i > 2*distance
print(1)
defaultVal = reg.predict([[0]])
