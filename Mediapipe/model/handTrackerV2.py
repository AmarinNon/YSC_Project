...
# cap = cv2.VideoCapture(r'D:\projectF\Video\1.mp4')
    ...
    if results.multi_hand_landmarks:
      for hand_landmarks in results.multi_hand_landmarks:
        ...
        cv2.putText(image,str(angle),
                tuple(np.multiply(Wrist,[width,heigth]).astype(int)),
        ...
        for id,lm in enumerate(hand_landmarks.landmark):
              ...
              if id==5:
                    ...
                    if tstart == "NO" and cy>upper_line and cy <down_line:
                    ...
                    if tstart != "NO" and checkStart == True:
                        ...
                        #   print(angle)
                          if abs(x-angle) <= 12:
                                stage = 'default'
                          if abs(x-angle) > 12 and stage =='default':
                                stage = 'change'
                                counter+=1
                                print("new:",counter)
                                print("X is:",x)
...
#     csvwriter.writerow({'Hz':counter/timer})
